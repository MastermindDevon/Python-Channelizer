...
def plot_complex_signal(inputFreq,samplingFreq,decimFactor,project_dir,module,options):
	...
	if (module=='IIRFilter'):
		...
		plt.subplot(312)
		plt.title('IIR Filter Magnitude')
		plt.xlabel('Time (s)')
		plt.ylabel('Magnitude')
		plt.plot(iirtpts,np.abs(iirPhasor),'r',label='IIR Filter Magnitude')
		...
		plt.plot(iirtpts,np.angle(iirPhasor)*(180/np.pi),'b',label='IIR Filter Phase')
		plt.legend(loc='lower right')
